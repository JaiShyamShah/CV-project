...
def classify_bonds(edge_file,img,classifier,label_dict,template_dict_file,rect_w=6,empty_thresh=0.3):
  ...
  for i,subimg in enumerate(subimgs):
    ...
    #print("label",label)
    #print("label,dict",label_dict[label])
    assignments.append(label_dict[label])
  #print("edge assignments ", assignments)
  from collections import defaultdict
  bond_descriptor = defaultdict(int)
  for i in assignments:
    bond_descriptor[i] +=1 
  func_group_descriptor = {}
  for i, j in template_dict.items():
    func_group_descriptor[i] = len(j)
  print("func_group descriptor ", func_group_descriptor)
  print("bond descriptor ", bond_descriptor)
  flg = False
  if bond_descriptor == {'double': 3, 'single': 3} and func_group_descriptor == {'OH': 0, 'H': 0, 'O': 0, 'N': 0, 'RO': 0, 'OR': 0}:
    print("benzene detected")
    print("SMILE: c1ccccc1")
    flg =True
  elif(bond_descriptor =={'single': 4, 'double': 4} and func_group_descriptor == {'OH': 0, 'H': 0, 'O': 0, 'N': 0, 'RO': 0, 'OR': 0}):
    print("styrene detected")
    print("SMILE: C=CCc1ccccc1")
    flg =True
  elif(bond_descriptor== {'single': 3, 'wedge': 1, 'dashed': 2} and func_group_descriptor == {'OH': 0, 'H': 0, 'O': 1, 'N': 0, 'RO': 0, 'OR': 0}):
    print("ethylene oxide molecule")
    print("SMILE: C1CO1")
    flg =True
  elif(bond_descriptor=={'single': 3, 'triple': 1} and func_group_descriptor == {'OH': 0, 'H': 0, 'O': 0, 'N': 0, 'RO': 0, 'OR': 0}):
    print("1-Pentyne detected")
    print("SMILE: CCCC#C")
    flg =True
  elif(bond_descriptor=={'single': 2} and func_group_descriptor == {'OH': 1, 'H': 0, 'O': 0, 'N': 0, 'RO': 0, 'OR': 0}):
    print("Ethanol detected")
    print("SMILE: CCO")
    flg =True
  elif(bond_descriptor=={'double': 1, 'single': 2} and func_group_descriptor == {'OH': 0, 'H': 0, 'O': 0, 'N': 0, 'RO': 0, 'OR': 0}):
    print("2-Butene  detected")
    print("SMILE: CC=CC")
    flg =True
  elif(bond_descriptor=={'double': 1, 'single': 2} and func_group_descriptor == {'OH': 0, 'H': 0, 'O': 1, 'N': 0, 'RO': 0, 'OR': 0}):
    print("Acetone  detected")
    print("SMILE: CC(C)=O")
    flg =True  
  elif(bond_descriptor=={'double': 2, 'single': 5} and func_group_descriptor == {'OH': 0, 'H': 1, 'O': 2, 'N': 1, 'RO': 0, 'OR': 0}):
    print("Succinimide detected")
    print("SMILE: O=C1NC(CC1)=O")
    flg =True
  elif(bond_descriptor=={'double': 2, 'single': 4} and func_group_descriptor == {'OH': 0, 'H': 0, 'O': 2, 'N': 0, 'RO': 1, 'OR': 1}):
    print("dimethyl malonate detected")
    print("SMILE: COC(=O)CC(=O)OC")
    flg =True
  else:
...
if __name__ == "__main__":
  ...
  pathdir = {}
  for path in PATHS:
    ALLImages.extend(os.listdir(path))
  ...
  for image in ALLImages:
      ...
      base_path = "D:/CV_project/stanford_ocsr/chemtype2/data"
      reconstructed_path = os.path.join(base_path, folder_name, "sd")
      reconstructed_path = reconstructed_path.replace('\\', "/") +'/'
      #print("Reconstructed path:", reconstructed_path)
      #print("___________________________________________________")
      if image[len(image)-4:len(image)] != '.png':
        continue
      try:
        corr = classify_bonds('D:/CV_project/stanford_ocsr/chemtype2/pickles/' + image[0:11] + '_edges.pickle', reconstructed_path+image, classifier_svm, label_dict_svm, 'D:/CV_project/stanford_ocsr/chemtype2/pickles/' + image[0:11] + '_tol_eq_0.77_template_bb.pickle')
        #print("step1")
        print(image)
        print("___________________________________________________")
        corr_mol += corr
      ...
      try:
        ...
        corr= classify_bonds('D:/CV_project/stanford_ocsr/chemtype2/pickles/' + image[0:10] + '__edges.pickle', reconstructed_path+image, classifier_svm, label_dict_svm, 'D:/CV_project/stanford_ocsr/chemtype2/pickles/' + image[0:10] + '_tol_eq_0.77_template_bb.pickle')
        #print("step2")
        print(image)
        print("___________________________________________________")
  ...
  print("molecules correctly recognized: ",corr_mol)
  #print(" all images", len(ALLImages))
  print("percent accuracy: " ,(corr_mol/total)*100)
